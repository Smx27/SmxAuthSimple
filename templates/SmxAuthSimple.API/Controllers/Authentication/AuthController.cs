...
using SmxAuthSimple.API.Controllers.DTO.Authentication;
using SmxAuthSimple.API.Data.Entities;
using SmxAuthSimple.API.Data.Interfaces;
using Swashbuckle.AspNetCore.Annotations;

namespace SmxAuthSimple.API.Controllers.Authentication;
/// <summary>
...
public class AuthController : ControllerBase
{
    ...
    /// <inheritdoc />
    public AuthController(UserManager<AppUser> userManager, IJwtTokenService tokenService, SignInManager<AppUser> signInManager)
    {
    ...
    public async Task<IActionResult> Login([FromBody] LoginDto loginDto)
    {
        var user = await _userManager.Users.SingleOrDefaultAsync(u =>
            (u.UserName != null && u.UserName.Equals(loginDto.UserName) || (u.Email != null && u.Email.Equals(loginDto.UserName))));
        if (user == null)
    ...
    private async Task<bool> ValidateRefreshToken(string refreshToken, string username)
    {
        ...
        return user?.RefreshToken != null && user.RefreshToken.Equals(refreshToken) && user.RefreshTokenExpiryTime >= DateTime.UtcNow;
    }
